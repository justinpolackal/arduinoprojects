...
void drawHome()
{
  ...
  tft.drawRoundRect(0, 0, 319, 240, 5, WHITE);     //Page border

  tft.fillRoundRect(60, 180, 200, 40, 5, RED);
  tft.drawRoundRect(60, 180, 200, 40, 5, WHITE);  //Game

  tft.fillRoundRect(60, 130, 200, 40, 5, RED);   //RGB led
  tft.drawRoundRect(60, 130, 200, 40, 5, WHITE);

  tft.fillRoundRect(60, 80, 200, 40, 5, RED);
  tft.drawRoundRect(60, 80, 200, 40, 5, WHITE); //Oscilloscope
...
void loop()
{
  ...
  if (currentpage == 0)
  {
    if (p.z > 10 && p.z < 1000)
    {
      if (p.x > 736 && p.x < 855 && p.y > 255 && p.y < 725  && p.z > MINPRESSURE && p.z < MAXPRESSURE)
      {
        ...
        tft.fillRoundRect(60, 180, 200, 40, 5, WHITE);

        delay(70);

        tft.fillRoundRect(60, 180, 200, 40, 5, RED);
        tft.drawRoundRect(60, 180, 200, 40, 5, WHITE);
        tft.setCursor(65, 195);
      ...
      else if (p.x > 563 && p.x < 683 && p.y > 275 && p.y < 750)
      {
        ...
        tft.fillRoundRect(60, 130, 200, 40, 5, WHITE);   //rgb led
        delay(70);

        tft.fillRoundRect(60, 130, 200, 40, 5, RED);   //rgb led
        tft.drawRoundRect(60, 130, 200, 40, 5, WHITE);   //rgb led
        tft.setCursor(105, 145);
      ...
      if (p.x > 403 && p.x < 525 && p.y > 271 && p.y < 725)
      {
        ...
        tft.fillRoundRect(60, 80, 200, 40, 5, WHITE);
        delay(70);

        tft.fillRoundRect(60, 80, 200, 40, 5, RED);
        tft.drawRoundRect(60, 80, 200, 40, 5, WHITE);
  ...
  if (currentpage == 1) //Flappy bird
  {
    ...
    // Process user input   If sensed a touch anywere on the screen eccept the area of the back and reset button
  ...
  if (currentpage == 2) // RGB mixer
  {
    ...
    if (p.z > MINPRESSURE && p.z < MAXPRESSURE)
    {
      ...
      {
        tft.fillRoundRect(5, 5, 50, 30, 5, WHITE);
        delay(70);
        tft.fillRoundRect(5, 5, 50, 30, 5, BLUE);
        tft.drawRoundRect(5, 5, 50, 30, 5, WHITE);
        tft.setCursor(15, 15);
        tft.print("<-");
        delay(70);
        tft.fillRoundRect(5, 5, 50, 30, 5, BLACK);
        currentpage = 0;
        drawHome();
        p.x = 160;
        p.y = 760;
...
void checkCollision() {
  ...
  if (crashed) {      //If it crashes somewere
    ...
    tft.setCursor(210, 145); //Set the cursor to 220,125
    tft.setTextSize(1);      //Set the text size to 5
    tft.setTextColor(WHITE);
